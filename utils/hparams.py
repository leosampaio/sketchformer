...
def combine_hparams_into_one(*args):
    """Combine different sets of hparams into a single giant one with all keys
    """
    combined_dict = args[0].values()
    for new_hps in args[1:]:
        combined_dict.update(new_hps.values())
    return HParams(**combined_dict)
